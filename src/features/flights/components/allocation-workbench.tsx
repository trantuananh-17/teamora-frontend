...
import { AllocationFlagBadge } from "@/components/status-badge"
...
import {
  useAllocationRuns,
  useCommitAllocation,
  useDiscardAllocation,
  useFlightAssignments,
  useFlights,
  useManualAssignFlight,
  usePreviewAllocation,
  useSetAssignmentLock,
    ...
    return registrations.items.map((registration) => {
      ...
      for (const direction of ["outbound", "return"] as const) {
        ...
        const planned = pendingRun.plan.assignments.find(
          (item) =>
            item.registrationId === registration.registrationId && item.direction === direction,
        )
  ...
  const columns = useMemo<ColumnDef<RegistrationFlight>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1">
            <span>
              {row.original.shiftPreference === "shift_1"
                ? "Ca 1"
                : row.original.shiftPreference === "shift_2"
                  ? "Ca 2"
                  : "—"}
            </span>
            {row.original.shiftLocked && <LockIcon className="size-3.5 text-muted-foreground" />}
          </div>
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <AssignmentCell
            assignment={row.original.assignments.outbound}
            onEdit={() => setManual({ row: row.original, direction: "outbound" })}
            onLock={() => {
              const item = row.original.assignments.outbound
              if (item) locker.mutate({ assignmentId: item.id, locked: !item.locked })
            }}
          />
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <AssignmentCell
            assignment={row.original.assignments.return}
            onEdit={() => setManual({ row: row.original, direction: "return" })}
            onLock={() => {
              const item = row.original.assignments.return
              if (item) locker.mutate({ assignmentId: item.id, locked: !item.locked })
            }}
          />
        ),
      },
      {
        ...
        cell: ({ row }) => {
          const flags = [
            ...new Set([
              ...(row.original.assignments.outbound?.flags ?? []),
              ...(row.original.assignments.return?.flags ?? []),
            ]),
          ]
          return (
            <div className="flex flex-wrap gap-1">
              {flags.length ? (
                flags.map((flag) => <AllocationFlagBadge key={flag} flag={flag} />)
              ) : (
  ...
  return (
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
          ...
          <StatCard
            ...
            value={
              ...
              flightsPage.items.reduce((sum, item) => sum + item.capacity, 0)
            }
            hint="Trong phương án gần nhất"
          />
          <StatCard
          ...
          <StatCard
            label="Lệch ca"
            value={<Count value={latest?.stats.shiftUnmet ?? 0} tone="warning" />}
            hint="Nguyện vọng không chiều được"
          />
        </div>
      }
      actions={
        <div className="flex flex-col gap-3 rounded-lg border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2 font-medium">
              {pendingRun ? "Đang xem phương án preview" : "Sẵn sàng tạo phương án mới"}
              {pendingRun && <Badge variant="secondary">Chưa áp dụng</Badge>}
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {pendingRun && (
              <>
                <Button variant="outline" onClick={() => setConfirmAction("discard")}>
                  <XIcon data-icon="inline-start" />
                  Bỏ phương án
                </Button>
                <Button onClick={() => setConfirmAction("commit")}>
    ...
    >
      {!canPreview && (
        <Alert className="m-4">
          <AlertTriangleIcon />
          <AlertTitle>Chưa thể phân bổ</AlertTitle>
          <AlertDescription>
            Cần có ít nhất một chuyến bay và một CBNV đăng ký tham gia.
          </AlertDescription>
        </Alert>
      )}
      <EntityDataTable
        ...
        emptyView={
          <EntityEmptyView
            ...
            message="Danh sách chỉ gồm CBNV đã chọn tham gia trong kỳ này."
          />
        }
      />
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {confirmAction === "commit" ? "Áp dụng phương án này?" : "Bỏ phương án preview?"}
            </DialogTitle>
            <DialogDescription>
              {confirmAction === "commit"
                ? "Hệ thống sẽ kiểm tra lại sức chứa và dữ liệu đăng ký, sau đó thay các phân bổ chưa khóa."
                : "Phương án nháp sẽ được đánh dấu đã bỏ; phân bổ hiện tại không thay đổi."}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const mutation = confirmAction === "commit" ? commit : discard
                mutation.mutate(pendingRun.id, { onSuccess: () => setConfirmAction(null) })
              }}
            >
              {confirmAction === "commit" ? "Áp dụng phương án" : "Bỏ phương án"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  return (
    <div className="flex min-w-40 items-center justify-between gap-2">
      <div>
        {assignment ? (
          <>
            <div className="font-medium">{assignment.flight.code}</div>
            <div className="text-xs text-muted-foreground">
              {assignment.source === "manual" ? "Thủ công" : "Tự động"}
              {assignment.locked ? " · Đã khóa" : ""}
            </div>
          </>
        ) : (
          <span className="text-sm text-muted-foreground">Chưa xếp</span>
        )}
      </div>
      <div className="flex">
        <IconAction label="Điều chỉnh chuyến" onClick={onEdit}>
          <UserRoundCogIcon />
        </IconAction>
        {assignment && (
          <IconAction
            ...
            onClick={onLock}
          >
            {assignment.locked ? <UnlockIcon /> : <LockIcon />}
          </IconAction>
        )}
      </div>
    </div>
...
function ManualAssignmentDialog({
  ...
  eventId: string
  selection: { row: RegistrationFlight; direction: FlightDirection } | null
  registrations: RegistrationFlight[]
  flights: {
    id: string
    code: string
    direction: FlightDirection
  ...
  const mutation = useManualAssignFlight(eventId, () => {
    setFlightId("")
    setWholeTeam(false)
    setReason("")
    onOpenChange(false)
  ...
  const ids = selection
    ? wholeTeam
      ? registrations
          .filter((row) => row.team.id === selection.row.team.id)
          .map((row) => row.registrationId)
      : [selection.row.registrationId]
    : []
  ...
  const alreadyOnTarget =
    selection && target
      ? registrations.filter(
          (row) =>
            ids.includes(row.registrationId) &&
            row.assignments[selection.direction]?.flight.id === target.id,
        ).length
      : 0
  ...
  return (
    <Dialog open={Boolean(selection)} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Điều chỉnh chuyến {selection?.direction === "outbound" ? "đi" : "về"}
          </DialogTitle>
          <DialogDescription>
            {selection?.row.user.name} · {selection?.row.team.name}. Điều chỉnh thủ công sẽ tự khóa
            để lần chạy sau không ghi đè.
          </DialogDescription>
        </DialogHeader>
        <FieldGroup>
          <Field>
            <FieldLabel>Chuyến đích</FieldLabel>
                ...
                <SelectGroup>
                  {candidates.map((flight) => (
                    <SelectItem key={flight.id} value={flight.id}>
                      {flight.code} · {flight.assignedCount}/{flight.capacity} chỗ
                    </SelectItem>
                  ))}
                </SelectGroup>
              </SelectContent>
          ...
          <Field orientation="horizontal">
            <div className="flex-1">
              <FieldLabel htmlFor="whole-team">Chuyển cả team</FieldLabel>
              <FieldDescription>
                {wholeTeam
                  ? "Toàn bộ thành viên Team sẽ được chuyển, kể cả người ở trang khác."
                  : "Chỉ điều chỉnh CBNV đang chọn."}
              </FieldDescription>
            </div>
            <Switch id="whole-team" checked={wholeTeam} onCheckedChange={setWholeTeam} />
          </Field>
          {overCapacity && (
            <Alert variant="destructive">
              <AlertTriangleIcon />
              <AlertTitle>Vượt sức chứa {finalOccupancy - target!.capacity} chỗ</AlertTitle>
              <AlertDescription>
                Bạn vẫn có thể xác nhận theo §5.5. Hệ thống sẽ lưu cảnh báo và lý do vào nhật ký.
              </AlertDescription>
            </Alert>
          )}
          <Field>
            <FieldLabel htmlFor="manual-reason">Lý do điều chỉnh</FieldLabel>
            <Textarea
              id="manual-reason"
              required
              maxLength={500}
        ...
        <DialogFooter>
          ...
          <Button
            disabled={!flightId || !reason.trim() || mutation.isPending}
            onClick={() =>
              mutation.mutate({
                registrationIds: wholeTeam ? undefined : ids,
                teamId: wholeTeam ? selection?.row.team.id : undefined,
                flightId,
                reason: reason.trim(),
              })
            }
