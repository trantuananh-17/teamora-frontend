...
export function AllocationWorkbench({ eventId, pagination }: { eventId: string; pagination: PageParams }) {
  ...
  const columns = useMemo<ColumnDef<RegistrationFlight>[]>(() => [
    ...
    { id: "flags", header: "Cảnh báo", cell: ({ row }) => { const flags = [...new Set([...row.original.assignments.outbound?.flags ?? [], ...row.original.assignments.return?.flags ?? []])]; return <div className="flex flex-wrap gap-1">{flags.length ? flags.map((flag) => <AllocationFlagBadge key={flag} flag={flag} />) : <span className="text-muted-foreground">—</span>}</div> }, meta: { priority: "tertiary" } },
  ], [locker])
  ...
  return (
    <EntityContainer
      ...
      stats={<div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
        <StatCard label="Lượt đã xếp" value={latest?.stats.assigned ?? 0} hint="Trong phương án gần nhất" />
        <StatCard label="Chưa xếp" value={<Count value={latest?.stats.unassigned ?? 0} tone="destructive" />} hint="Cần xử lý trước" />
        <StatCard label="Chỗ còn lại" value={latest?.stats.remainingSlots ?? flightsPage.items.reduce((sum, item) => sum + item.capacity, 0)} hint="Trong phương án gần nhất" />
        <StatCard label="Team bị tách" value={<Count value={latest?.stats.teamsSplit ?? 0} tone="warning" />} hint="Cần kiểm tra" />
        <StatCard label="Lệch ca" value={<Count value={latest?.stats.shiftUnmet ?? 0} tone="warning" />} hint="Nguyện vọng không chiều được" />
      </div>}
...
function IconAction({ label, onClick, children }: { label: string; onClick: () => void; children: React.ReactNode }) {
  return <Tooltip><TooltipTrigger asChild><Button size="icon-sm" variant="ghost" aria-label={label} onClick={onClick}>{children}</Button></TooltipTrigger><TooltipContent>{label}</TooltipContent></Tooltip>
}

function AssignmentCell({ assignment, onEdit, onLock }: { assignment: Assignment; onEdit: () => void; onLock: () => void }) {
  return <div className="flex min-w-40 items-center justify-between gap-2"><div>{assignment ? <><div className="font-medium">{assignment.flight.code}</div><div className="text-xs text-muted-foreground">{assignment.source === "manual" ? "Thủ công" : "Tự động"}{assignment.locked ? " · Đã khóa" : ""}</div></> : <span className="text-sm text-muted-foreground">Chưa xếp</span>}</div><div className="flex"><IconAction label="Điều chỉnh chuyến" onClick={onEdit}><UserRoundCogIcon /></IconAction>{assignment && <IconAction label={assignment.locked ? "Mở khóa để lần chạy sau được xếp lại" : "Khóa, giữ nguyên qua mọi lần chạy lại"} onClick={onLock}>{assignment.locked ? <UnlockIcon /> : <LockIcon />}</IconAction>}</div></div>
}
...
function Count({ value, tone }: { value: number; tone: "warning" | "destructive" }) {
  return <span className={value ? (tone === "warning" ? "text-warning" : "text-destructive") : undefined}>{value}</span>
}
