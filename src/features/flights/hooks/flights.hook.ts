...
import {
  allocationRunsOptions,
  flightAssignmentsOptions,
  flightsKeys,
  flightsListOptions,
} from "../options/flights.options"
import {
  commitAllocation,
  createFlight,
  deleteFlight,
...
export const useAllocationRuns = (eventId: string) =>
  useSuspenseQuery(allocationRunsOptions(eventId))

function useFlightMutation<T>(
  eventId: string,
...
export const useCreateFlight = (eventId: string, onDone?: () => void) =>
  useFlightMutation<FlightInput>(
    eventId,
    (input) => createFlight(eventId, input),
    "Đã thêm chuyến bay",
    onDone,
  )
export const useUpdateFlight = (eventId: string, flightId: string, onDone?: () => void) =>
  useFlightMutation<FlightInput>(
    eventId,
    (input) => updateFlight(eventId, flightId, input),
...
export const useImportFlights = (eventId: string) =>
  useFlightMutation<File>(
    eventId,
    (file) => importFlights(eventId, file),
    "Đã import danh sách chuyến bay",
  )
...
export const useCommitAllocation = (eventId: string) =>
  useFlightMutation<string>(
    eventId,
    (id) => commitAllocation(eventId, id),
    "Đã áp dụng phương án phân chuyến",
  )
export const useDiscardAllocation = (eventId: string) =>
  useFlightMutation<string>(
    eventId,
    (id) => discardAllocation(eventId, id),
    "Đã bỏ phương án preview",
  )
export function useManualAssignFlight(eventId: string, onDone?: () => void) {
  ...
  return useMutation({
    ...
    onError: async (error) =>
      toast.error("Không điều chỉnh được", { description: await errorMessage(error) }),
  })
}
export const useSetAssignmentLock = (eventId: string) =>
  useFlightMutation<{ assignmentId: string; locked: boolean }>(
    eventId,
    ({ assignmentId, locked }) =>
      setAssignmentLock(eventId, assignmentId, locked, "Khóa để giữ nguyên khi chạy lại phân bổ"),
    "Đã cập nhật khóa phân bổ",
  )
