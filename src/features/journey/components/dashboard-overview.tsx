      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
          {Object.entries(data.transportNeeds).map(([leg, total]) => (
            <Badge key={leg} variant="outline" className="px-3 py-1.5">
              {legLabels[leg] ?? leg}: {total}
            </Badge>
          ))}
        </CardContent>
      </Card>
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            {data.capacity.flights.map((item) => (
              <Utilization
                key={item.id}
                label={item.code}
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            {data.capacity.vehicles.map((item) => (
              <Utilization
                key={item.id}
                label={item.code}
                assigned={item.assigned}
