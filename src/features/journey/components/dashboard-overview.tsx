...
function Utilization({ label, assigned, capacity }: { label: string; assigned: number; capacity: number }) { const percent = capacity ? Math.round(assigned / capacity * 100) : 0; return <div className="flex flex-col gap-1"><div className="flex justify-between gap-3 text-sm"><span className="truncate">{label}</span><span className="tabular-nums text-muted-foreground">{assigned}/{capacity} · {percent}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${percent > 100 ? "bg-destructive" : "bg-primary"}`} style={{ width: `${Math.min(100, percent)}%` }} /></div></div> }

export function DashboardOverview({ eventId }: { eventId: string }) { const { data } = useDashboard(eventId); return <div className="flex flex-col gap-6">
  ...
  <div className="grid gap-4 lg:grid-cols-3">
    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><PlaneIcon className="size-4" />Chuyến bay</CardTitle></CardHeader><CardContent className="flex flex-col gap-3">{data.capacity.flights.map((item) => <Utilization key={item.id} label={item.code} assigned={item.assigned} capacity={item.capacity} />)}</CardContent></Card>
    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><BusIcon className="size-4" />Xe</CardTitle></CardHeader><CardContent className="flex flex-col gap-3">{data.capacity.vehicles.map((item) => <Utilization key={item.id} label={item.code} assigned={item.assigned} capacity={item.capacity} />)}</CardContent></Card>
    <Card><CardHeader><CardTitle className="flex items-center gap-2 text-base"><BedDoubleIcon className="size-4" />Phòng</CardTitle></CardHeader><CardContent className="flex flex-col gap-3">{data.capacity.rooms.map((item) => <Utilization key={item.id} label={`${item.hotel} · ${item.code}`} assigned={item.assigned} capacity={item.capacity} />)}</CardContent></Card>
  </div>
</div> }
